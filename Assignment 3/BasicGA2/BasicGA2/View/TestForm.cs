    ...
    public partial class TestForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < myGA.offspring.Count(); i++)
                richTextBox4.AppendText(myGA.offspring[i].value.ToString() + " " + Math.Round(myGA.offspring[i].fitness, 2).ToString() + "\n");

            myGA.mutationStep();
            label5.Text = "mutationStep" + myGA.countmutation;
            for (int i = 0; i < myGA.population.Count(); i++)
                richTextBox5.AppendText(myGA.population[i].value.ToString() + " " + Math.Round(myGA.population[i].fitness, 2).ToString() + "\n");

            myGA.updateStep();
            label6.Text = "updateStep";
            for (int i = 0; i < myGA.population.Count(); i++)
                richTextBox6.AppendText(myGA.population[i].value.ToString() + " " + Math.Round(myGA.population[i].fitness, 2).ToString() + "\n");
