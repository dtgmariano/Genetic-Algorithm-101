...
\title{\vspace{-15mm}\fontsize{24pt}{10pt}\selectfont\textbf{Algoritmo genético básico para otimização de soluções em um problema de busca do valor mínimo da função de Schwefel}} % Article title
...
A função \ref{eq:probfunc} é uma função tridimensional, possuindo diversos mínimos e máximos locais. Para o problema foi determinado o domínio [0, 500] para valores que x e y podem assumir.
...
Como a equação do problema tem como entrada duas variáveis - X e Y - foi criado um cromossomo de tamanho 2*N, onde os primeiros N bits do cromossomo representam o valor da variável X e os últimos N bits representam o valor da variável Y.
Para uma representaçao de 10 bits, com x = 23 e y = 37, teríamos o seguinte cromossomo:
\begin{figure}[H]
  \caption{Representação cromossomial para o problema.}
  \centering
    \includegraphics[scale = 1]{chrom_represent_schwefel.png}
\end{figure}
