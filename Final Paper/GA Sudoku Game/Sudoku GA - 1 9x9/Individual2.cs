...
namespace GA
{
    public class Individual2
    {
        ...
        public Individual2(Random _random)
        {
        ...
        public Individual2(List<int> _gene)
        {
            if (_gene.Count() > 81)
            {
                Console.Write(true);
        ...
        int CalcColumnPenalty(List<int> _gene, int columnIndex)
        {
            ...
            for (int i = columnIndex; i < side; i+=side)
            {
                int value = _gene[i];
                bool containsItem = testList.Any(item => item == value);
